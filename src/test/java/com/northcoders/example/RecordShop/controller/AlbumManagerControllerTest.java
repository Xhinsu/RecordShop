...
class AlbumManagerControllerTest {
    ...
    void getAll() throws Exception {
        ...
        albumList.add(new Album(1,"Midnight","Taylor"));
        albumList.add(new Album(2,"Morning","Smith"));
        albumList.add(new Album(3,"Afternoon","Seamstress"));

        int albumListSize = albumList.size();

        //this is mocking controller getAll method and it returns albumList
        when(albumManagerController.getAll()).thenReturn(albumList);
        ...
        this.mockMvcController.perform(MockMvcRequestBuilders.get("/api/album"))
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("$[1].name").value("Morning"))
                .andExpect(MockMvcResultMatchers.jsonPath("$[3].artist").value("Seamstress"));
    }
    ...
    void getAlbumbyId() throws Exception {
        Album album = new Album(1,"Midnight","Taylor");


        //this is mocking controller getAll method and it returns albumList from fake repo interaction
        when(albumManagerController.getAlbumById(1)).thenReturn(album);
